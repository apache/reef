...
/**
 * ControllerActivity Controls the matrix multiplication task Splits up the
 * input matrix into parts(row-wise) and scatters them amongst the compute
 * activities Broadcasts each column vector Receives the reduced(concatenated)
 * partial products as the output vector
 * 
 * @author shravan
 * 
 */
public class ControllerActivity implements Activity {
  private final Logger logger = Logger.getLogger(ControllerActivity.class
      .getName());
	/**
	 * The Group Communication Operators that are needed by this activity. These
	 * will be injected into the constructor by TANG. The operators used here
	 * are complementary to the ones used in the ComputeActivity
	 */
	 ...
	 */
	@Inject
	public ControllerActivity(Scatter.Sender<Vector> scatterSender,
			Broadcast.Sender<Vector> broadcastSender,
			Reduce.Receiver<Vector> reduceReceiver) {
		super();
	...
	public byte[] call(byte[] memento) throws Exception {
	  ...
	  logger.log(Level.FINE, "Scattering A");
		...
		for (Vector x : X) {
			...
			// partial sums computed by each computeActivity
