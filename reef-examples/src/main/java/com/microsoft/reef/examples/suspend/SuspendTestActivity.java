...
/**
 * Simple do-nothing activity that can send messages to the Driver and can be suspended/resumed.
 */
@Unit
public class SuspendTestActivity implements Activity, ActivityMessageSource {

  /**
   * Standard java logger.
   */
  private static final Logger LOG = Logger.getLogger(SuspendTestActivity.class.getName());
  private final CheckpointService checkpointService;
  /**
   * number of cycles to run in the activity.
   */
  private final int numCycles;
  /**
   * delay in milliseconds between cycles in the activity.
   */
  ...
  /**
   * Main method of the activity: run cycle from 0 to numCycles,
  ...
  /**
   * Update driver on current state of the activity.
   *
   * @return serialized version of the counter.
   */
  @Override
  public synchronized Optional<ActivityMessage> getMessage() {
    LOG.log(Level.INFO, "Message from Activity {0} to the Driver: counter: {1}",
            new Object[]{this, this.counter});
    return Optional.of(ActivityMessage.from(SuspendTestActivity.class.getName(), this.codecInt.encode(this.counter)));
  }

  /**
   * Save current state of the activity in the checkpoint.
   *
  ...
  /**
   * Restore the activity state from the given checkpoint.
   *
  ...
  public class SuspendHandler implements EventHandler<SuspendEvent> {
    ...
    public void onNext(SuspendEvent suspendEvent) {
      ...
      LOG.log(Level.INFO, "Suspend: {0} with: {1} bytes; counter: {2}",
              new Object[]{this, message.length, SuspendTestActivity.this.counter});
      SuspendTestActivity.this.suspended = true;
      SuspendTestActivity.this.notify();
    }
  }
