...
Application developers interact with hand-written modules by invoking ad-hoc builder methods, and manually passing configurations (in the correct order) from module to module.  In practice, hand-written modules frequently delgate to each other, either via inheritance or via wrapper classes and chains of method invocations.  This makes it extremely difficult for developers and end-users to infer what the final value of a given configuration paramter will be at runtime, or even to figure out why a particular option was (or was not) set after an application runs.

Tang strongly discourages the use of code-based modules that execute at runtime.  Instead, Tang allows application developers to use data structures called `ConfigurationModule`s instead of code based modules:

- Configurations and ConfigurationModules are "just data," and can be read and written in human readable formats.
- Interfaces and configuration parameters are encouraged to specify sane defaults, allowing production code to generate minimalistic configuration files.  This makes it extremely easy for developers, end-users and system-administrators to see what's "strange" about a particular, problematic application instance.
- Because Tang modules are "just data" that is generated before the application runs, Tang's static analysis and documentation tools sanity check modules, and document their side effects and any additional parameters they surface.
...
- Therefore, the results of most Tang configuration operations are independent of the order in which configurations are composed with each other at runtime, greatly simplfying application-level configuration and bootstrapping logic.
...
- ConfigurationModules, which are discussed above, are statically specified data structures that allow applications to modularize configuration data, and that support a wide-range of runtime and build-time checks.

Each of these formats simply emits Tang Configurations, which are then composed (in a monotonic, order-independent way) at runtime.
...
```
will perform full static analysis of all classes the class path, and emit a nicely formatted HTML document.  To documentation generated by Tint includes cross-references between configuration options, interfaces, classes, and the `ConfigurationModules` that use and set them. 
