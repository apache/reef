...
/**
 * Driver for TaskScheduler. It receives the commands by HttpRequest and execute
 * them in a FIFO(First In First Out) order.
 */
@Unit
public final class SchedulerWithLocalityDriver {

  public static final ObjectSerializableCodec<String> CODEC = new ObjectSerializableCodec<>();
  private static final Logger LOG = Logger.getLogger(SchedulerWithLocalityDriver.class.getName());
   ...
   * evaluator.</dd>
   * <dt><code>WAIT_EVALUATORS</code></dt><dd>Waiting for an evaluator allocated
   * with no active evaluators.</dd>
   ...
   */
  private final boolean retainable;

  private State state = State.INIT;

  private final SchedulerWithLocality scheduler;
  ...
  @Inject
  private SchedulerWithLocalityDriver(final EvaluatorRequestor requestor,
          @Parameter(SchedulerREEF.Retain.class) final boolean retainable,
          final SchedulerWithLocality scheduler) {
   ...
   * Non-retainable version, while occurs only once in the Retainable version
   */
  public final class EvaluatorAllocatedHandler implements EventHandler<AllocatedEvaluator> {
    ...
    public void onNext(final AllocatedEvaluator evaluator) {
      ...
      synchronized (SchedulerWithLocalityDriver.this) {
        nActiveEval++;
        nRequestedEval--;
      }
  ...
  /**
   * Now it is ready to schedule tasks. But if the queue is empty, wait until
   * commands coming up.
   *
   * If there is no pending task, having more than 1 evaluators must be
   * redundant. It may happen, for example, when tasks are canceled during
   * allocation. In these cases, the new evaluator may be abandoned.
   */
  ...
  /**
   * When a Task completes, the task is marked as finished. The evaluator is
   * reused for the next Task if retainable is set to {@code true}. Otherwise
   * the evaluator is released.
   */
  ...
  /**
   * Cancel a Task waiting on the queue. A task cannot be canceled once it is
   * running.
   */
  ...
  public synchronized int submitCommand(final String command) {
    ...
    scheduler.addTask(new TaskPathEntity(new TaskEntity(id, command), ""));
  ...
  /**
   * Update the maximum number of evaluators to hold. Request more evaluators in
   * case there are pending tasks in the queue and the number of evaluators is
   * less than the limit.
   */
  ...
  private synchronized void requestEvaluator(final String path) throws IOException {
    ...
    while (listLocatedStatus.hasNext()) {
      ...
      for (BlockLocation blockLocation : next.getBlockLocations()) {
        String nodenameToTarget = blockLocation.getHosts()[0];

        nRequestedEval += 1;
        requestor.submit(EvaluatorRequest.newBuilder()
                .setMemory(32)
                .addNodeName(nodenameToTarget)
                .build());
      }
    }
  ...
  /**
   * Request evaluators. Passing a non positive number is illegal, so it does
   * not make a trial for that situation.
   */
  ...
  /**
   * Pick up a command from the queue and run it. Wait until any command coming
   * up if no command exists.
   */
  ...
  /**
   * Retain the complete evaluators submitting another task until there is no
   * need to reuse them.
   */
  ...
  /**
   * Always close the complete evaluators and allocate a new evaluator if
   * necessary.
   */
