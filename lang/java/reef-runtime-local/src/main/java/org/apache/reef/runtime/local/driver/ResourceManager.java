...
public final class ResourceManager {
  ...
  private final Set<String> racksOnNetwork;
  ...
  /**
   * Check if at least one requested rack is available in the "network", otherwise throw an exception
  ...
  private void validateRackNames(final List<String> rackNames) {
    for (final String rackName : rackNames) {
      if (racksOnNetwork.contains(rackName)) {
        return;
      }
    }
    throw new IllegalArgumentException("Racks requested for Evaluators do not exist in the network");
  }
  ...
  private void checkRequestQueue() {
    ...
    if (requestQueue.hasOutStandingRequests()) {
      ...
      for (final String rackName : rackNames) {
        ...
        // if we allocated it or we do not want to try on other racks as the relax locality constraint is disabled,
        // then break, don't try to allocate on the other racks
        if (allocated || Boolean.FALSE.equals(requestEvent.getRelaxLocality().get())) {
          break;
        } else {
          LOG.log(Level.FINEST, "Could not allocate on rack {0}, trying on another one", rackName);
        }
      }
      // update REEF
