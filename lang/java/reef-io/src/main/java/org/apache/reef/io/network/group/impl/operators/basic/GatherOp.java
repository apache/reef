...
/**
 * Implementation of {@link org.apache.reef.io.network.group.operators.Gather}
 */
public class GatherOp implements Gather {
  ...
  public final static class Sender<T> extends SenderBase<T> implements Gather.Sender<T> {
    ...
    public void send(final T element) throws NetworkException, InterruptedException {
      this.dataSender.send(getSelf(), getParent(), element, ReefNetworkGroupCommProtos.GroupCommMessage.Type.Gather);
      final String result = this.ackReceiver.receive(getParent(), getSelf(), ReefNetworkGroupCommProtos.GroupCommMessage.Type.Gather);
  ...
  public static class Receiver<T> extends ReceiverBase<T> implements Gather.Receiver<T> {
    ...
    public List<T> receive(final List<? extends Identifier> order)
      ...
      final List<T> result = this.dataReceiver.receive(order, getSelf(), ReefNetworkGroupCommProtos.GroupCommMessage.Type.Gather);
      LOG.log(Level.FINEST, "{0} received: {1} from {2}",
          new Object[]{getSelf(), result, order});
      for (final Identifier child : order) {
        this.ackSender.send(getSelf(), child, "ACK", ReefNetworkGroupCommProtos.GroupCommMessage.Type.Gather);
      }
      return result;
    }
  }
