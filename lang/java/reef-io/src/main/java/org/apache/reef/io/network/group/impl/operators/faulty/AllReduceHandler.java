...
public class AllReduceHandler implements EventHandler<Message<ReefNetworkGroupCommProtos.GroupCommMessage>> {
  ...
  private final ConcurrentHashMap<Identifier,
      BlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>> id2que = new ConcurrentHashMap<>();
  ...
  public AllReduceHandler(
    ...
    for (final String id : ids) {
      ...
      this.id2que.put(compId, new LinkedBlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>());
  ...
  public <T> T get(final Identifier id,
    ...
    final ReefNetworkGroupCommProtos.GroupCommMessage gcm = this.id2que.get(id).take();
