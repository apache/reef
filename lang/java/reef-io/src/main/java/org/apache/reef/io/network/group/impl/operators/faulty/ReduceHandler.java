...
public final class ReduceHandler implements EventHandler<Message<ReefNetworkGroupCommProtos.GroupCommMessage>> {
  ...
  private final ConcurrentHashMap<Identifier,
      BlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>> id2dataQue = new ConcurrentHashMap<>();
  ...
  public ReduceHandler(
      final @Parameter(BroadRedHandler.IDs.class) Set<String> ids,
  ...
  public synchronized void addChild(final Identifier compId) {
    ...
    this.id2dataQue.put(compId, new LinkedBlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>());
  }
  ...
  @Override
  public void onNext(final Message<ReefNetworkGroupCommProtos.GroupCommMessage> value) {

    final ReefNetworkGroupCommProtos.GroupCommMessage oneVal = value.getData().iterator().hasNext() ?
  ...
  public void sync(final Map<Identifier, Integer> isIdAlive
    ...
    while (!this.ctrlQue.isEmpty()) {
      ...
      final ReefNetworkGroupCommProtos.GroupCommMessage gcm = this.ctrlQue.poll();
      final Identifier id = this.idFac.getNewInstance(gcm.getSrcid());
      final int status = isIdAlive.containsKey(id) ? isIdAlive.get(id) : 0;

      if (gcm.getType() == ReefNetworkGroupCommProtos.GroupCommMessage.Type.SourceAdd) {
  ...
  public <T> T get(final Identifier id,
    ...
    final ReefNetworkGroupCommProtos.GroupCommMessage gcm = id2dataQue.get(id).take();
    if (gcm.getType() == ReefNetworkGroupCommProtos.GroupCommMessage.Type.SourceDead) {
      LOG.log(Level.FINEST, "\t\tGot src dead msg from driver. Terminating wait and returning null");
      return null;
    }

    T retVal = null;
    for (final ReefNetworkGroupCommProtos.GroupMessageBody body : gcm.getMsgsList()) {
