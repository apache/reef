...
import org.apache.reef.io.network.group.impl.operators.faulty.BroadRedHandler;
import org.apache.reef.io.network.group.impl.operators.faulty.BroadReduceConfig;
...
public class BRManager {
  ...
  public BRManager(
    ...
    this.ns = new NetworkService<>(this.idFac, 0, nameServiceAddr, nameServicePort, new GCMCodec(),
        new MessagingTransportFactory(), new EventHandler<Message<ReefNetworkGroupCommProtos.GroupCommMessage>>() {
      @Override
      public void onNext(final Message<ReefNetworkGroupCommProtos.GroupCommMessage> srcAddsMsg) {
        final ReefNetworkGroupCommProtos.GroupCommMessage srcAdds = srcAddsMsg.getData().iterator().next();
        assert (srcAdds.getType() == ReefNetworkGroupCommProtos.GroupCommMessage.Type.SourceAdd);
        final SingleThreadStage<ReefNetworkGroupCommProtos.GroupCommMessage> sendReqSrcAdd =
            new SingleThreadStage<>(new EventHandler<ReefNetworkGroupCommProtos.GroupCommMessage>() {
              @Override
              public void onNext(final ReefNetworkGroupCommProtos.GroupCommMessage srcAddsInner) {
                final SerializableCodec<HashSet<Integer>> sc = new SerializableCodec<>();
                for (final ReefNetworkGroupCommProtos.GroupMessageBody body : srcAddsInner.getMsgsList()) {
                  ...
                  for (final int src : srcs) {
                    ...
                    BRManager.this.srcAdds.putIfAbsent(srcId, new LinkedBlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>(1));
                    final BlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage> msgQue = BRManager.this.srcAdds.get(srcId);
                    try {
                      LOG.log(Level.FINEST, "Waiting for srcAdd msg from: {0}", srcId);
                      final ReefNetworkGroupCommProtos.GroupCommMessage srcAddMsg = msgQue.take();
    ...
    this.senderStage = new ThreadPoolStage<>(
        "SrcCtrlMsgSender", new EventHandler<ReefNetworkGroupCommProtos.GroupCommMessage>() {
      @Override
      public void onNext(final ReefNetworkGroupCommProtos.GroupCommMessage srcCtrlMsg) {

        final Identifier id = BRManager.this.idFac.getNewInstance(srcCtrlMsg.getDestid());

        if (BRManager.this.tree.getStatus((ComparableIdentifier) id) != TaskTree.Status.SCHEDULED) {
          return;
        }

        final Connection<ReefNetworkGroupCommProtos.GroupCommMessage> link = BRManager.this.ns.newConnection(id);
        try {
          link.open();
  ...
  public Configuration getControllerContextConf(final ComparableIdentifier id) {
  ...
  public Configuration getControllerActConf(final ComparableIdentifier taskId) {
  ...
  public Configuration getComputeContextConf(final ComparableIdentifier taskId) {
    final JavaConfigurationBuilder jcb = tang.newConfigurationBuilder(this.reduceBaseConf);
    jcb.addConfiguration(createNetworkServiceConf(
  ...
  public Configuration getComputeActConf(final ComparableIdentifier taskId) {
    final JavaConfigurationBuilder jcb = tang.newConfigurationBuilder();//reduceBaseConf);
  ...
  private Configuration createHandlerConf(
      final List<ComparableIdentifier> ids) {
  ...
  public synchronized void schedule(final ComparableIdentifier taskId, final boolean reschedule) {
    ...
    if (TaskTree.Status.SCHEDULED == this.tree.getStatus(taskId)) {
      return;
    }

    this.tree.setStatus(taskId, TaskTree.Status.SCHEDULED);
  ...
  private void sendSrcAddMsg(final ComparableIdentifier from,
    ...
    final ReefNetworkGroupCommProtos.GroupCommMessage srcAddMsg = Utils.bldGCM(
        ReefNetworkGroupCommProtos.GroupCommMessage.Type.SourceAdd, from, to, new byte[0]);

    if (reschedule) {
      LOG.log(Level.FINEST, "SrcAdd from: {0} queued up", from);
      this.srcAdds.putIfAbsent(from, new LinkedBlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>(1));
      final BlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage> msgQue = this.srcAdds.get(from);
  ...
  public List<ComparableIdentifier> tasksToSchedule(final ComparableIdentifier runTaskId) {
    ...
    for (final ComparableIdentifier child : children) {
      if (TaskTree.Status.COMPLETED == this.tree.getStatus(child)) {
  ...
  public synchronized void unschedule(final ComparableIdentifier failedTaskId) {
    ...
    this.tree.setStatus(failedTaskId, TaskTree.Status.UNSCHEDULED);
  ...
  private void sendSrcDeadMsg(final ComparableIdentifier from, final ComparableIdentifier to) {
    final ReefNetworkGroupCommProtos.GroupCommMessage srcDeadMsg = Utils.bldGCM(
        ReefNetworkGroupCommProtos.GroupCommMessage.Type.SourceDead, from, to, new byte[0]);
  ...
  public void complete(final ComparableIdentifier id) {
    ...
    this.tree.setStatus(id, TaskTree.Status.COMPLETED);
  }
}
