 ...
 * {@link Message}s  of type {@link org.apache.reef.io.network.proto.ReefNetworkGroupCommProtos.GroupCommMessage}
 * <p/>
 * Uses a {@link Handler} per operators to receive from each registered task
 * Essentially a {@link org.apache.reef.io.network.proto.ReefNetworkGroupCommProtos.GroupCommMessage} sent from a task using an operator will
...
public class GroupCommNetworkHandler implements EventHandler<Message<ReefNetworkGroupCommProtos.GroupCommMessage>> {
  ...
  private static class GCMHandler implements Handler {
    ...
    public GCMHandler(List<Identifier> ids, IdentifierFactory factory, int capacity) {
      idQues = new HashMap<Identifier, BlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>>(ids.size());
      idFactory = factory;
      this.capacity = capacity;
      for (Identifier id : ids) {
        idQues.put(id, new LinkedBlockingQueue<ReefNetworkGroupCommProtos.GroupCommMessage>(this.capacity));
      }
    }
     ...
     */
    @Override
    public ReefNetworkGroupCommProtos.GroupCommMessage getData(Identifier srcId) throws InterruptedException {
     ...
     */
    @Override
    public void onNext(ReefNetworkGroupCommProtos.GroupCommMessage data) {
  ...
  public GroupCommNetworkHandler(List<Identifier> ids, IdentifierFactory factory, int capacity) {
    ...
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.AllGather, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.AllReduce, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.Broadcast, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.Gather, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.Reduce, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.ReduceScatter, new GCMHandler(ids, factory, capacity));
    handlerMap.put(ReefNetworkGroupCommProtos.GroupCommMessage.Type.Scatter, new GCMHandler(ids, factory, capacity));
  }
  ...
  public void onNext(Message<ReefNetworkGroupCommProtos.GroupCommMessage> msg) {
    ReefNetworkGroupCommProtos.GroupCommMessage data = msg.getData().iterator().next();
