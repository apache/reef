...
import org.apache.reef.io.network.group.operators.Scatter;
...
/**
 * Implementation of {@link org.apache.reef.io.network.group.operators.Scatter}
 */
public class ScatterOp implements Scatter {

  private static final Logger LOG = Logger.getLogger(ScatterOp.class.getName());
  ...
  public static final class Sender<T> extends SenderBase<T> implements Scatter.Sender<T> {
    ...
    public void send(
      ...
      this.dataSender.send(getSelf(), order, elements, counts, ReefNetworkGroupCommProtos.GroupCommMessage.Type.Scatter);
      final List<String> result = this.ackReceiver.receive(order, getSelf(), ReefNetworkGroupCommProtos.GroupCommMessage.Type.Scatter);
  ...
  public static final class Receiver<T> extends ReceiverBase<T> implements Scatter.Receiver<T> {
    ...
    public List<T> receive() throws InterruptedException, NetworkException {
      final List<T> result = this.dataReceiver.receiveList(getParent(), getSelf(), ReefNetworkGroupCommProtos.GroupCommMessage.Type.Scatter);
      LOG.log(Level.FINEST, "{0} received: {1} from {2}",
          new Object[]{getSelf(), result, getParent()});
      this.ackSender.send(getSelf(), getParent(), "ACK", ReefNetworkGroupCommProtos.GroupCommMessage.Type.Scatter);
      return result;
    }
  }
}
