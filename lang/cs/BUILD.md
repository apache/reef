...
4. Git clone the repo to your local machine
    ```
    git clone git@github.com:apache/reef.git
    ```
5. Build and test REEF .Net using Maven
    ```
    mvn clean install
    ```
    Or to perform build alone without tests in a multithreaded mode, execute
    ```
    mvn -TC1 -DskipTests clean install
    ```
    Or to perform "fast" build, which skips tests and all code quality enforcement tools, execute:
    ```
