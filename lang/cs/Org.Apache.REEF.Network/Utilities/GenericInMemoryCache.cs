...
namespace Org.Apache.REEF.Network.Utilities
{
    ...
    /// <summary>
    /// This is a generic cache subsystem based on key/value pairs, where key is generic, and must be unique.
    /// Every cache entry has its own timeout and such an associated Timer.
    /// Cache is thread safe and will delete expired entries on its own using System.Threading.Timers which uses the ThreadPool construct).
    /// </summary>
    public class GenericInMemoryCache<K, T> : IDisposable
    {
        #region Constructor and class members
        /// <summary>
        /// Initializes a new instance of the <see cref="Cache{K,T}"/> class.
        /// </summary>
        public GenericInMemoryCache()
        {
        }

        /// <summary>
        /// Definitions for the internal cache, the timers and the write lock object
        /// </summary>
        ...
        #region CheckTimer
        // Checks whether a specific timer already exists and adds a new one, when needed
        private void CheckTimer(K key, int cacheTimeout, bool restartTimerIfExists)
        {
            ...
            else
            {
                timers.Add(
                    key,
                    new Timer(new TimerCallback(RemoveByTimer), key, cacheTimeout == Timeout.Infinite ? Timeout.Infinite : cacheTimeout, Timeout.Infinite));
            }
        }
        ...
        public T Get(K key)
        {
            ...
            try
            {
                T rv = cache.TryGetValue(key, out rv) ? rv : default(T);
                return rv;
            }
            finally
            {
    ...
    /// </summary>
    public class GenericInMemoryCache : GenericInMemoryCache<string, object>
    {
        #region Static Global Cache instance 
        private static Lazy<GenericInMemoryCache> global = new Lazy<GenericInMemoryCache>();
        ...
        /// </value>
        public static GenericInMemoryCache Global = global.Value;
        #endregion
    }
    #endregion
}