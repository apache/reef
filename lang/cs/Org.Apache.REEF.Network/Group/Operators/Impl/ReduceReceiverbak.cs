...
namespace Org.Apache.REEF.Network.Group.Operators.Impl
{
    ...
    public class ReduceReceiver<T> : IReduceReceiver<T>
    {
        ...
        public T Reduce()
        {
            LOGGER.Log(Level.Info, "*****************in reduce receiving message from child*************8");
            var data =_topology.ReceiveFromChildren(ReduceFunction);
            LOGGER.Log(Level.Info, "*****************in reduce done receiving message from child and reduce *************8");
            return data;
        }
    }
}
