...
namespace Org.Apache.REEF.Network.Examples.Elastic
{
    ...
    /// </summary>
    public sealed class ElasticBroadcastDriverWithFailureBeforeWorkflow : ElasticBroadcastDriverWithFailures
    {
        [Inject]
        private ElasticBroadcastDriverWithFailureBeforeWorkflow(
            ...
            get
            {
                return (taskId) => TangFactory.GetTang().NewConfigurationBuilder(
                    Context.GetTaskConfigurationModule(taskId)
                        .Set(TaskConfiguration.Task, GenericType<BroadcastSlaveTaskDieBeforeWorkflow>.Class)
                        .Build())
                    .Build();
            }
        }
