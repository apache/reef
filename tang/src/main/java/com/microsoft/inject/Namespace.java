...
public class Namespace {
		...
		public ClassNode(Class<?> clazz, boolean isPrefixTarget) {
			...
			for (int k = 0; k < constructors.length; k++) {
				...
				if (injectAllConstructors
					...
					// For some reason, if the length of this array is > 0, then
					// it's prepended with a pointer to the constructed
					// object... Pull that parameter off the list. If there are
					// zero parameters, then the returned array has length zero,
					// instead of 1 (which would at least be consistent...)
					/*
					...
					if (paramTypes.length == paramAnnotations.length + 1) {
						// TODO: For some reason, this is sometimes needed, and
						// sometimes not...
					...
					ConstructorDef def = new ConstructorDef(args);
					if (injectableConstructors.contains(def)) {
						throw new IllegalStateException(
								"Ambiguous constructors detected in class "
										+ clazz + ": " + def
	...
	class NamedParameterNode extends Node {
		final NamedParameter namedParameter;
		...
		NamedParameterNode(NamedParameter n) {
			...
			try {
				...
				throw new IllegalArgumentException("Named parameter "
						+ n.toString() + " takes unknown class as argument: "
						+ n.type() + ".");
			}
		}
	...
	class ConstructorArg {
		...
		@Override
		public String toString() {
			return name == null ? type.getSimpleName() : (type.getSimpleName()
					+ " " + name.value());
		}
	}
	...
	class ConstructorDef {
		...
		ConstructorDef(ConstructorArg[] args) {
			this.args = args;
		...
		/**
		 * Check to see if two constructors take indistinguishable arguments. If
		 * so (and they are in the same class), then this would lead to
		 * ambiguous injection targets, and we want to fail fast.
		 * 
	...
	public void findUnresolvedClasses(Node root, Set<Class<?>> unresolved) {
		if (root instanceof ClassNode) {
			...
			if(zuper != null && getNode(zuper) == null) unresolved.add(zuper);
			Class<?>[] interfaces = cls.clazz.getInterfaces();
			for(Class<?> i : interfaces) {
				if(getNode(i) == null) unresolved.add(i);
			}
		}
	...
	public void resolveAllClasses() {
		for (Class<?>[] classes = findUnresolvedClasses();
		     classes.length > 0;
			 classes = findUnresolvedClasses()) {
			for (Class<?> c : classes) {
				registerClass(c);
			}
		}
