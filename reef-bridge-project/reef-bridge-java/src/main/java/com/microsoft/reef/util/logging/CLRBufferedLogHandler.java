...
public class CLRBufferedLogHandler extends Handler
{
    ...
    private void logAll()
    {
        synchronized (this) {
            for (LogRecord record : this.logs) {
                int logLevel = getLevel(record);
                String message = formatter.format(record);

                try {
                    NativeInterop.ClrBufferedLog(logLevel, message);
                }
                catch (Exception e) {
                    System.err.println("Failed to perform CLRBufferedLogHandler for bridge: " + message);
                }
            }
     ...
     */
    private int getLevel(LogRecord record)
    {
